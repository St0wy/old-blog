...
I won't go into too much details about the computer graphics concepts applied here. If that's something of interest to you,
you can read [LearnOpenGL](https://learnopengl.com/) which is where I learned everything needed to make this scene real!

![Picture of the demo scene of my renderer](/opengl-scene/chat.jpg)
...
As I still wanted to support models that have 3 separate textures for the ARM maps, I decided to check if the model has an ARM map.
But to my suprise, Assimp has option to check this, which means the only way to know is to check the texture type `aiTextureType_UNKNOWN`.
To make things worse, this ARM map counts as a roughness map and as a metalness.
So if you're not carefull, you might load the map 3 times.
...
This is why I then decided to use an other format : KTX.
...
but this is horrible in terms of performance, so I decided to go on an other direction.
...
It also keeps all of the data next to eachother, and this makes the structure more cache friendly.
This means that my scene graph looks something like this :

![graph on how the scene graph is layed out](/opengl-scene/scene-graph.svg)

And what's nice with this code, is that traversing the tree to draw it to the screen is not that much more complex than using some good old DFS.
...
As you can see, we first loop over the graph, and each object is added to a hash map. This map has basically the material and mesh ids as a key.
...
For example, one material could have an ambient occlusion map and an other one might not.
...
And fortunately, it's possible ! You just have to multiply the the `lightViewProjMatrix` by the inverse of the camera view matrix when you create it.
...
One thing I really struggled with, is having wierd shadows although the math seemed correct.
...
![fragPos](/opengl-scene/fragPos.png)
![ambientOcclusion](/opengl-scene/ambientOcclusion.png)

Then in the second attachment, the normals in view-space and the roughness is stored.

![normal](/opengl-scene/normal.png)
![roughness](/opengl-scene/roughness.png)

And in the third attachment, we have the base color and the metallic.

![baseColor](/opengl-scene/baseColor.png)
![metallic](/opengl-scene/metallic.png)
...
![ssao](/opengl-scene/ssao.png)

Then, the 4 levels of the cascaded shadow maps are rendered. (I'll only show two of them.)

![lightDepth](/opengl-scene/lightDepth.png)
![lightDepth2](/opengl-scene/lightDepth2.png)
...
![ambientLighting](/opengl-scene/ambient.png)

Then the point light (yes it's very similar) :

![pointLight](/opengl-scene/pointLight.png)

And finally the directional light :

![directionalLight](/opengl-scene/directional.png)

Now that the light computation is done, sphere that are the color of the lights are rendered on top of the attachment and the skybox is drawn.

![skybox](/opengl-scene/skybox.png)

Then bloom is rendered.

![bloom](/opengl-scene/bloom.png)

And now we can mix the bloom with the previous framebuffer and apply some HDR tonemapping and gamma correction.
(In my case I use Narkowicz ACES).

![final](/opengl-scene/final.png)
...
*Now some bonus funny visual bugs*

![bob omb](/opengl-scene/bobomb.jpg)
![cursed](/opengl-scene/cursed.png)
![spike](/opengl-scene/spike.bmp)

*and the full ibl developped integral because why not*

![cursed](/opengl-scene/ibl.png)
