...
I learned to program in C#, and visual studio really did all of the hard work in term of handling the build so I never had to think about it.
...
![Schema of the project.](/build-systems/project.png)
...
I just added the option to be able to call the script with with `release` afterwards to build with optimisations on.
...
The executable has a pretty similar script, just with different flags to be able to link against the the library.
...
I never actually wrote a Powershell script in my life, so I thought it could be a good idea to see how
...
Basically, to find the cpp files, I was able to use the `Get-ChildItem` command instead of the wierd for loop.
...
Another nice thing is that variables are immutable by default and that the syntax is more terse than Powershell.
...
An other nice thing from Nushell is that it generates an help page from these parameters.

![Help page of the build command.](/build-systems/help.png)
...
This isn't stricly necessary, but I did it just to be on the safe side.
...
And now we will build each cpp file separately into an o file.
...
As you can see we use now the split compiler.
...
But if you're familliar with Nushell, you might wonder why I don't use the syntax with `^` to run the command ?
...
I lost so much time debugging wierd things when I used Nushell and Zig that it made me like CMake.
...
but even without having using them, I something tells me I will have a bad time trying to use a CMake only library with them.
