...
Well, that means that everyone at school was using USB Tethering from their phone, via 4G or 5G.
...
This wasn't too annoying since I could just configure the server from home, but it made troubleshooting a pain.

## GitLab

![Screenshot of GitLab](/src/assets/devops/gitlab.png)
...
The only annoying thing was that it was struggling to get certificates to be able to use HTTPS, so I had to use [Certbot](https://certbot.eff.org/) to manually generate them.

After GitLab was installed, I created the repositories for each project and then created accounts for everyone.
Unfortunately, Gmail blocked my server (probably to avoid spams), so no one received the mail from GitLab and I sent everyone a temporary password that they had to change.
...
![Screenshot of Nextcloud](/src/assets/devops/nextcloud.png)
...
So I wanted for the Nextcloud data (by that I mean the files of the users) to be on my HDD (1 TO).
...
When we thought we had lost all hope, the WiFi came back at school ! 
We thought this meant that all of our problem where fixed, but we were quite naive.
On the new network, IPv6 isn't enabled, the SSH port is blocked, 
so no one can clone SSH repos (not even on GitHub) and the school's server is on another sub-network, meaning that we cannot access it.
...
but I these tasks were the ones where I had the most fun.
